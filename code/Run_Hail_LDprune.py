...
outdir='/project/nmancuso_8/elezhang/projects/FA/data/PanUKBB/3_ldprune/'+args.dat+'/pruned/'
indir='/project/nmancuso_8/elezhang/projects/FA/data/PanUKBB/3_ldprune/'+args.dat+'/toprune/'
chr=args.chr
# finished:
# running:
# smaller: 8,9,10,13,14,15,16,17,18,19,20
# larger: 1,2,3,4,5,6,7, 11, 12
...
def ld_prune(ht_idx, r2_bm, bp_window_size, keep_higher_maf=True):
    ...
    info = ht_idx.aggregate(hl.agg.collect(ht_idx.row.select('idx', *fields)), _localize=False)
    ...
    entries = entries.annotate(
        i=hl.struct(idx=entries.i,
                    maf=entries.info[entries.i].maf),
        j=hl.struct(idx=entries.j,
                    maf=entries.info[entries.j].maf))
    print("done preparing entries")

    variants_to_remove = hl.maximal_independent_set(entries.i, entries.j, keep=False, tie_breaker=tie_breaker, keyed=False)
    print("done find variants to remove")

    ht_idx = ht_idx.annotate_globals(variants_to_remove=variants_to_remove.aggregate(hl.agg.collect_as_set(variants_to_remove.node.idx), _localize=False))

    ## remove rows that is pruned out
    ht_idx_pruned = ht_idx.filter(ht_idx.variants_to_remove.contains(hl.int32(ht_idx.idx)), keep=False).select()
...
}, default_reference='GRCh37', min_block_size=128)
