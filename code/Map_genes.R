...
library(tidyverse)
setwd("/project/nmancuso_8/elezhang/projects/FA/data/PanUKBB/3_ldprune/sig_pval5e-08_nct2_maf1e-02_1kg/varinfo")

# Reference (all approved gene symbol)
ref <- read_tsv("/project/nmancuso_8/data/GENE_MAP/hgnc_complete_set_20221219.txt") %>% 
...
## extract all gene names from Previous_symbols and Alias_symbols
# old_names <- ref_approved %>% filter(!is.na(Previous_symbols)) %>% pull(Previous_symbols)
# alias_names <- ref_approved %>% filter(!is.na(Alias_symbols)) %>% pull(Alias_symbols)

## expand rows
# all_gene_symbol <- unlist(strsplit(gsub("\\s", "", old_names), split = ","))
# all_gene_symbol <- c(all_gene_symbol, unlist(strsplit(gsub("\\s", "", alias_names), split = ",")))
# all_gene_symbol <- unique(all_gene_symbol)

# pat <- paste0("\\b", all_gene_symbol, "\\b", collapse = "|")  
# 
# ref_approved_old <- ref_approved %>%
#   mutate(Previous_symbols = stringr::str_extract_all(Previous_symbols, pat)) %>% 
#   tidyr::unnest(Previous_symbols)
# 
# ref_approved_alias <- ref_approved %>%
#   mutate(Alias_symbols = stringr::str_extract_all(Alias_symbols, pat)) %>% 
#   tidyr::unnest(Alias_symbols) 
