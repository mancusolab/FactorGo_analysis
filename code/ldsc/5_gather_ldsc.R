...
annot_name <- args[1] # SEG
filelist <- args[2] # param_SEG_file_annot
facnum <- args[3] # onefac, all
focal <- as.logical(args[4]) # True, False

resdir <- paste0("./results/PanUKBB/sig_pval5e-08_nct2_maf1e-02_1kg/enrich/",
                 annot_name, "/ldsc_cts/")
outdir <- paste0("./results/PanUKBB/sig_pval5e-08_nct2_maf1e-02_1kg/enrich/",
                 annot_name, "/")
